...
scattersmooth <-
function (x, y, nbin = 100, lambda = 1, ndot = 500, csize = 0.3, 
          ticks = TRUE, xlim = c(min(x), max(x)), ylim = c(min(y), 
                                                           max(y)), show = TRUE, save = FALSE, data = NULL, xlab = NULL, 
          ylab = NULL, cols = heat.colors(10:200), col.points = "blue", axes=TRUE,...) 
{
  smooth2d = function(H, lambda) {
    ...
    D2x <- diff(D1x)
    D2y <- diff(D1y)
    Qx <- E1 + lambda[1]^2 * t(D2x) %*% D2x + 2 * lambda[1] * 
      t(D1x) %*% D1x
    Qy <- E2 + lambda[2]^2 * t(D2y) %*% D2y + 2 * lambda[2] * 
      t(D1y) %*% D1y
...
if(is.null(xlab)){xlab <- deparse(substitute(x))}
  ...
  if (all(!is.numeric(lambda)) | all(lambda < 0) | (length(lambda) > 
                                                      2)) 
    stop("lambda should be numeric and positive")
  if (length(lambda) == 1) 
    lambda <- c(lambda, lambda)
  if (all(!is.numeric(nbin)) | all(nbin < 1) | (length(nbin) > 
                                                  2)) 
  ...
  sel <- 1 <= xbin & xbin <= nbin[1] & 1 <= ybin & ybin <= 
    nbin[2]
  xbin <- xbin[sel]
  ybin <- ybin[sel]
  ...
  if (ticks == FALSE) 
    par(xaxt = "n", yaxt = "n")
  if (show) {
    image(x = xgrid, y = ygrid, z = -Hsmooth, xlab = xlab, 
          ylab = ylab, col = cols,axes=axes)
    if (ndot > 0) {
      ndot <- min(m, ndot)
      sel <- sort.list(rnorm(m))[1:ndot]
      points(x[sel], y[sel], cex = csize, col = col.points, 
             pch = 15)
    }
    box()
  }
  sg <- list()
  if (save == TRUE) {
    sg <- list(Hraw = Hraw, Hsmooth = Hsmooth, xgrid = xgrid, 
               ygrid = ygrid, xbin = xbin, ybin = ybin, nmiss = nmiss, 
               seldots = sel)
    sg
  }
  else {
